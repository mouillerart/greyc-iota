...
public class SCDispositionRule extends SCRule {

    private static final Log log = LogFactory.getLog(SCDispositionRule.class);
    public static final String RULEFILTER = "Disposition";

    @Override
    protected String getId() {
        return "urn:oasis:names:tc:xacml:1.0:resource:disposition-id";
    }

    SCDispositionRule(String name, List dispositions, OneOrGlobalFunction dispositionsFilterFunction) {
        super(name, dispositions, dispositionsFilterFunction);
        description = "Disposition filter for SC: " + this.name;
    }
    ...
    public SCDispositionRule(String ruleId, List values) {
        super(ruleId, values);
    }

    public SCDispositionRule(String ruleId, List values, String actions) {
