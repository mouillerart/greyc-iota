...
public class SCBizStepRule extends SCRule {

    private static final Log log = LogFactory.getLog(SCBizStepRule.class);
    public static final String RULEFILTER = "BizStep";

    @Override
    protected String getId() {
        return "urn:oasis:names:tc:xacml:1.0:resource:bizStep-id";
    }

    SCBizStepRule(String name, List bizSteps, OneOrGlobalFunction bizStepsFilterFunction) {
        super(name, bizSteps, bizStepsFilterFunction);
        description = "BizStep filter for SC: " + this.name;
    }
    ...
    public SCBizStepRule(String ruleId, List values) {
        super(ruleId, values);
    }

    public SCBizStepRule(String ruleId, List values, String actions) {
