...
public class SCBizTransRule extends SCRule {

    private static final Log log = LogFactory.getLog(SCBizTransRule.class);

    @Override
    protected String getId() {
        return "urn:oasis:names:tc:xacml:1.0:resource:bizTrans";
    }
    public static final String RULEFILTER = "BizTrans";

    SCBizTransRule(String name, List bizTrans, OneOrGlobalFunction bizTransFilterFunction) {
        super(name, bizTrans, bizTransFilterFunction);
        description = "BizTrans filter for SC: " + this.name;
    }
    ...
    public SCBizTransRule(String ruleId, List values) {
        super(ruleId, values);
    }

    public SCBizTransRule(String ruleId, List values, String actions) {
