...
public class SCEpcsRule extends SCRule {

    private static final Log log = LogFactory.getLog(SCEpcsRule.class);
    public static final String RULEFILTER = "Epc";

    @Override
    protected String getId() {
        return "urn:oasis:names:tc:xacml:1.0:resource:epc-id";
    }

    public SCEpcsRule(String ruleId, List values) {
        super(ruleId, values);
        description = "EPCs filter for SC: " + this.name;
    }

    public SCEpcsRule(String ruleId, List values, String actions) {
        super(ruleId, values, actions);
        description = "EPCs filter for SC: " + this.name;
    }

    SCEpcsRule(String name, List epcs, OneOrGlobalFunction epcsFilterFunction) {
        super(name, epcs, epcsFilterFunction);
        description = "EPCs filter for SC: " + this.name;
    }
