        ...
        if (ownerPolicies != null) {
            for (Object ogp : ownerPolicies.getPolicies()) {
                if (ogp instanceof GroupPolicy) {
                    ...
                    MasterDataIdRuleTreeNode masterDataIdRuleTreeNode = new MasterDataIdRuleTreeNode(gp.getMasterDataIdsFilterFunction(), gp.getId().toString(), Module.queryModule);
                    for (Object ob : gp.getMasterDataIds()) {
                        String value = (String) ob;
                        MasterDataIdTreeNode treeNode = new MasterDataIdTreeNode(value, value, gp.getId().toString(), Module.queryModule);
                        masterDataIdRuleTreeNode.addChild(treeNode);
                    }
                    filters.addChild(masterDataIdRuleTreeNode);
        ...
        if (ownerPolicies != null) {
            for (Object ogp : ownerPolicies.getPolicies()) {
                if (ogp instanceof GroupPolicy) {
                    ...
                    MasterDataIdRuleTreeNode masterDataIdRuleTreeNode = new MasterDataIdRuleTreeNode(gp.getMasterDataIdsFilterFunction(), gp.getId().toString(), Module.captureModule);
                    for (Object ob : gp.getMasterDataIds()) {
                        String value = (String) ob;
                        MasterDataIdTreeNode treeNode = new MasterDataIdTreeNode(value, value, gp.getId().toString(), Module.captureModule);
                        masterDataIdRuleTreeNode.addChild(treeNode);
                    }
                    filters.addChild(masterDataIdRuleTreeNode);
...
//            createCapturePolicies(sessionId, owner);
    ...
    public synchronized TreeNode createGroupPolicy(GroupPolicy gp, Module module) {
        ...
        if (!(module.equals(Module.adminModule))) {
            ...
            MasterDataIdRuleTreeNode masterDataIdRuleTreeNode = new MasterDataIdRuleTreeNode(gp.getMasterDataIdsFilterFunction(), gp.getId().toString(), module);
            for (Object ob : gp.getMasterDataIds()) {
                String value = (String) ob;
                MasterDataIdTreeNode treeNode = new MasterDataIdTreeNode(value, value, gp.getId().toString(), module);
                masterDataIdRuleTreeNode.addChild(treeNode);
            }
            filters.addChild(masterDataIdRuleTreeNode);
