...
public class UserOperationsModule {
    ...
    public void userCreate(String sessionId, String login, String password, String partner)
        ...
        int resp = epcispep.userCreate(userId, partner, Module.administrationModule.getValue());
        if (resp != Result.DECISION_PERMIT && !epcispep.isRootAccess(sessionId, Module.administrationModule.getValue())) {
    ...
    public void userDelete(String sessionId, String uid)
        ...
        for (User user : userList) {
            int resp = epcispep.userDelete(userId, user.getPartnerID(), Module.administrationModule.getValue());
            if (resp != Result.DECISION_PERMIT && !epcispep.isRootAccess(sessionId, Module.administrationModule.getValue())) {
                String msg = "Acces denied.";
                SecurityException se = new SecurityException();
                se.setReason(msg);
    ...
    public List<User> userLookup(String sessionId, String userId)
        ...
        try {
            ...
            while (iterUser.hasNext()) {
                ...
                int resp = epcispep.userLookup(u.getUserID(), user.getPartnerID(), Module.administrationModule.getValue());
                if (resp != Result.DECISION_PERMIT && !epcispep.isRootAccess(sessionId, Module.administrationModule.getValue())) {
                    String msg = "Acces denied.";
                    SecurityException se = new SecurityException();
                    se.setReason(msg);
    ...
    public User userInfo(String sessionId, String userId)
        ...
        try {
            ...
            User u = Session.getUser(sessionId);
            int resp = epcispep.userInfo(u.getUserID(), uList.get(0).getPartnerID(), Module.administrationModule.getValue());
            if (resp != Result.DECISION_PERMIT && !epcispep.isRootAccess(sessionId, Module.administrationModule.getValue())) {
                String msg = "Acces denied.";
                SecurityException se = new SecurityException();
                se.setReason(msg);
