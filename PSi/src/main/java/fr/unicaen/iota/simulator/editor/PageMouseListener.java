 ...
 * PageMouseListener
 *
 * This class handels the mouseevents on an editors page. Move objects on
 * mousemotionevents. Create, delete, edit objects on mouseclickevents.
 *
 * Created after de.huberlin.informatik.pnk.editor.PageMouseListener
 */
class PageMouseListener implements MouseListener, MouseMotionListener, ActionListener, ItemListener {
    ...
    /**
     * If mousepointer is over a sprite, so emphasize the sprite. This stores
    ...
    /**
     * if popumenu is visible, the value will be true so the next mouseclick
     * will only destroy the popupmenu and not init some other actions on page,
     * like creating a place or a transition.
     */
    private boolean popupVisible = false;
    /**
     * Aktiviert den Gravitationsmodus beim plazieren und Bewegen der Sprites
     */
    ...
    /**
     * If Destroy button pushed, close page. If Print button pushed, print page.
     */
    @Override
    public void actionPerformed(ActionEvent e) {
        String cmd = e.getActionCommand();
        if ("Close Page".equals(cmd)) {
            this.close();
        } else if ("Print".equals(cmd)) {
            this.page.print();
        } else if ("Grav".equals(cmd)) {
            this.grav();
        }
    }
    ...
    private void addArc(Sprite s) {
        ...
        if (!(s instanceof MemberSpriteNode)) {
            ...
            this.highlightInitial((MemberSprite) this.initialnode, initAction);
    ...
    /**
     * Sets extensions to sprite, if sprite is a netobject.
     */
    private void addExtensions(Sprite sprite) {
        ...
        Map<String, String> extensions = graph.getExtensionIdToValue(netobj);
        if (extensions == null) {
            return;
        }
        int extCount = 0;
        for (Map.Entry<String, String> idval : extensions.entrySet()) {
            ...
            Point labelpos = new Point(sprite.getBounds().getLocation());
            int dx = sprite.getSize().width;
            int dy = (extCount++) * fm.getHeight();

            if ("marking".equals(id)) {
                size = new Dimension(305, 200);
                labelpos.translate((int) (-size.getWidth() + sprite.getBounds().getWidth()) / 2, (int) (-size.getHeight() + sprite.getBounds().getHeight()) / 2);
            } else {
                labelpos.translate(dx, dy);
            }

            // Create extensionlabel, add it to page
            Extension ext = new Extension(sprite, labelpos, size, fm, id, value, page, null);
            if ("epcgenerator".equals(ext.getId())) {
                ext.setType(RepresentationType.PICTURE);
                ext.setPicturePath("./pictures/test.png");
            } else if ("pipe".equals(ext.getId())) {
                ext.setType(RepresentationType.PICTURE);
                ext.setPicturePath("./pictures/pipe.png");
            } else if ("epcdeactivator".equals(ext.getId())) {
                ext.setType(RepresentationType.PICTURE);
                ext.setPicturePath("./pictures/trash.png");
            }
    ...
    private void close() {
        ...
        int ok;
        if (this.page.getSpritevector().isEmpty()) {
            ok = 0;
        } else {
            ok = this.page.getEditor().showConfirmDialog("Delete all objects on page?");
        }
        if (ok == 0) {
            // OK - Button was pressed -> delete all objects
            Iterable<Sprite> v = new ArrayList<Sprite>(this.page.getSpritevector());
            for (Sprite s : v) {
                if (s instanceof MemberSpriteNode) {
                    this.split((MemberSpriteNode) s);
                }
                this.delete(s);
            }
            this.page.close();
    ...
    /**
     * Deletes a sprite. Removes sprite from page. using the page
     * remove(sprite)-method. If sprite is a MemberSprite it also will be remove
     * in editors Referencetable. If it is last MemberSprite of a kernel.Nets
     * object the Referencetable will delete netobject in kernel.Net
     * automaticaly
     */
    ...
    private void emphasize(Sprite s) {
        ...
        if (this.emphasizedSprite == s) {
            ...
            for (Sprite s2 : this.emphasizedSprite.subsprites) {
                s2.setHightLighted(true);
                if (s2 instanceof Representation) {
                    Representation r = (Representation) s2;
                    updateRegion = updateRegion.union(r.getBounds());
                }
            }
    ...
    private void grav() {
        ...
        for (Sprite sprite : this.page.getSpritevector()) {
        ...
        if (sprite == null) {
            if (editable) {     // check if editor permitts this operation
                ...
                if (this.initialnode != null) {
                    ...
                    this.addTransition(point);
                } else if (em.isInMode(EditorMenu.PLACE)) {
                    this.addPlace(point);
                } else if (em.isInMode(EditorMenu.NODE)) {
                    this.addNode(point);
                }
                if (this.gravitationOn) {
                    this.grav();
            ...
            if (this.page.getEditor().selectOneObject) {
                this.selectOneObject(sprite);
            } else if (em.isInMode(EditorMenu.SELECT)) {
                this.select(sprite);
            } else if (editable && em.isInMode(EditorMenu.EDGE)) {
                this.addEdge(sprite);
            } else if (editable && em.isInMode(EditorMenu.ARC)) {
                this.addArc(sprite);
            } else if (editable && em.isInMode(EditorMenu.EDIT)) {
                this.edit(sprite);
            } else if (editable && em.isInMode(EditorMenu.DELETE)) {
                this.delete(sprite);
            } else if (editable && em.isInMode(EditorMenu.JOIN)) {
                this.join(sprite);
            } else if (editable && em.isInMode(EditorMenu.SPLIT)) {
                this.split(sprite);
            }
        }
    }
    ...
    /**
     * Inverse selectionStatus of sprite. Sets the sprite as selected if it is
     * not selected. Sets the sprite as not selected if it is selected.
     */
    ...
    /**
     * Editor is waiting for a netobject selected on pages. Sets the selected
     * Netobject in editor.
     */
    ...
    private void showPopup(MouseEvent e) {
        ...
        if (currentobject == null) {
             ...
             * ckb.addItemListener(this); this.popup.add(ckb);
             */
            //deletemodus
    ...
    private void split(Sprite sprite) {
        if (sprite instanceof MemberSprite) {
            ...
            ReferenceTable.SpritePageTuple tupel;
            Object netobject = msprite.getNetobject();

            // check if split is possible, else return
            info = rtable.get(netobject);
            if (info.spritelist.size() < 2) {
                return;
            }
