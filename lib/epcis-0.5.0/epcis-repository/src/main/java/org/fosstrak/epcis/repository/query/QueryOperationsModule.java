...
public class QueryOperationsModule implements EpcisQueryControlInterface {
    ...
    private void refreshSubscriptions() {
        try {
            ...
            Map<String, QuerySubscriptionScheduled> map = loadSubscriptions(session);
            saveSubscriptions(map);
            session.close();
        } catch (Exception e) {
            // SQLException from openSession()
            // SQLException or ImplementationExceptionResponse from loadSubscriptions()
            // Wrapping it in a RuntimeException to forward it to the servlet
            // contener without having to modify callers API.
            throw new RuntimeException("Error while refreshing subscriptions: " + e.getMessage(), e);
        }
    }
