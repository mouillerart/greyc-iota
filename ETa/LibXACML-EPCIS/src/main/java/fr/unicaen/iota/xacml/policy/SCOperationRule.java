...
public class SCOperationRule extends SCRule {

    private static final Log log = LogFactory.getLog(SCOperationRule.class);
    public static final String RULEFILTER = "Operation";

    @Override
    protected String getId() {
        return "urn:oasis:names:tc:xacml:1.0:resource:operation-id";
    }

    public SCOperationRule(String ruleId, List values) {
        super(ruleId, values);
        description = "Operation filter for SC: " + this.name;
    }

    public SCOperationRule(String ruleId, List values, String actions) {
        super(ruleId, values, actions);
        description = "Operation filter for SC: " + this.name;
    }

    SCOperationRule(String name, List operations, OneOrGlobalFunction operationsFilterFunction) {
        super(name, operations, operationsFilterFunction);
        description = "Operation filter for SC: " + this.name;
    }
