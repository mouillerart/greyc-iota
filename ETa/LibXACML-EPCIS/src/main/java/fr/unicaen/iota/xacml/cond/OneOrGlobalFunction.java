...
public class OneOrGlobalFunction extends FunctionBase {
    ...
    public static int getId(String functionName) {
        if (NAME_GLOBAL_DENY.equals(functionName)) {
            return ID_GLOBAL_DENY;
        } else if (NAME_GLOBAL_PERMIT.equals(functionName)) {
            return ID_GLOBAL_PERMIT;
        } else {
            throw new IllegalArgumentException("unknown one or global function: " + functionName);
        }
    }
