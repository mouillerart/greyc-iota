...
public class SCChildEpcRule extends SCRule {

    private static final Log log = LogFactory.getLog(SCChildEpcRule.class);
    public static final String RULEFILTER = "ChildEpc";

    @Override
    protected String getId() {
        return "urn:oasis:names:tc:xacml:1.0:resource:childEpc-id";
    }

    public SCChildEpcRule(String ruleId, List values) {
        super(ruleId, values);
        description = "ChildEpc filter for SC: " + this.name;
    }

    public SCChildEpcRule(String ruleId, List values, String actions) {
        super(ruleId, values, actions);
        description = "ChildEpc filter for SC: " + this.name;
    }

    SCChildEpcRule(String name, List childEpcs, OneOrGlobalFunction childEpcsFilterFunction) {
        super(name, childEpcs, childEpcsFilterFunction);
        description = "ChildEpc filter for SC: " + this.name;
    }
