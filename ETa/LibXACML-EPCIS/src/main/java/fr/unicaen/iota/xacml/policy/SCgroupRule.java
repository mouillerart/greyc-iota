...
public class SCgroupRule extends SCRule {
    
    private static final Log log = LogFactory.getLog(SCgroupRule.class);
    public static final String RULEFILTER = "UserGroup";
 
    @Override
    protected String getId() {
        return "urn:oasis:names:tc:xacml:1.0:subject:user-id";
    }
    
    public SCgroupRule(String name, List users) {
        super(name, users);
        description = "Rule for SC group access: " + this.name;
    }
    
    public SCgroupRule(String name, List users, OneOrGlobalFunction f) {
        super(name, users, f);
        description = "Rule for SC group access: " + this.name;
    }
    
    public SCgroupRule(String name, List users, String actions) {
        super(name, users, actions);
        description = "Rule for SC group access: " + this.name;
    }
    ...
    protected Condition createCondition() {
        ...
        for (Object user : values) {
            ...
            AttributeDesignator designator = new AttributeDesignator(AttributeDesignator.SUBJECT_TARGET, designatorType, designatorId, true, null);
            applyArgsUser.add(designator);
            Apply applyUser = new Apply(applyFunction, applyArgsUser);
            applyArgs.add(applyUser);
            StringAttribute value = new StringAttribute((String) user);
            applyArgs.add(value);
            Apply applyCondition = new Apply(conditionFunction2, applyArgs);
            conditionArgs.add(applyCondition);
        }
        for (Object o : conditionArgs) {
            Apply ap = (Apply) o;
        }
