 ...
 *
 */
public class SCEventTimeRule extends SCRule {

    private static final Log log = LogFactory.getLog(SCEventTimeRule.class);
    public static String RULEFILTER = "EventTime";

    @Override
    protected String getId() {
        return "urn:oasis:names:tc:xacml:1.0:resource:eventTime-id";
    }

    public SCEventTimeRule(String ruleId, List values) throws Exception {
        super(ruleId, values);
        description = "EventTime filter for SC: " + this.name;
        if (values.size() % 2 != 0) {
            throw new Exception("values have to be added in couple");
        }
    }

    public SCEventTimeRule(String ruleId, List values, String actions) throws Exception {
    ...
    SCEventTimeRule(String name, List eventTimes, OneOrGlobalFunction eventTimesFilterFunction) {
        super(name, eventTimes, eventTimesFilterFunction);
        description = "EventTime filter for SC: " + this.name;
    }
    ...
    protected Condition createCondition() {
        ...
        Function greaterFunction = null;
        try {
            greaterFunction = factory.createFunction(ComparisonFunction.NAME_DATETIME_GREATER_THAN_OR_EQUAL);
        } catch (Exception e) {
            log.error("Exception in condition creation", e);
            return null;
        }

        Function lessFunction = null;
        try {
        ...
        try {
            dateDesignatorId = new URI("urn:oasis:names:tc:xacml:1.0:resource:eventTime-id");
        } catch (URISyntaxException ex) {
            log.fatal(null, ex);
        }
        ...
        try {
            ...
            log.error("Exception in SCEventTime condition", e);
            return null;
        }
        ...
        for (Object datesOb : values) {
            ...
            AttributeDesignator lowDesignator = new AttributeDesignator(AttributeDesignator.RESOURCE_TARGET, designatorType, dateDesignatorId, true, null);
            ...
            AttributeDesignator greatDesignator = new AttributeDesignator(AttributeDesignator.RESOURCE_TARGET, designatorType, dateDesignatorId, true, null);
