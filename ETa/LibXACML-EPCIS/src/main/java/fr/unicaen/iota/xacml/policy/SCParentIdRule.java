...
public class SCParentIdRule extends SCRule {

    private static final Log log = LogFactory.getLog(SCParentIdRule.class);
    public static final String RULEFILTER = "ParentId";

    @Override
    protected String getId() {
        return "urn:oasis:names:tc:xacml:1.0:resource:parentId-id";
    }

    public SCParentIdRule(String ruleId, List values) {
        super(ruleId, values);
        description = "ParentId filter for SC: " + this.name;
    }

    public SCParentIdRule(String ruleId, List values, String actions) {
        super(ruleId, values, actions);
        description = "ParentId filter for SC: " + this.name;
    }

    SCParentIdRule(String name, List parentIds, OneOrGlobalFunction parentIdsFilterFunction) {
        super(name, parentIds, parentIdsFilterFunction);
        description = "ParentId filter for SC: " + this.name;
    }
